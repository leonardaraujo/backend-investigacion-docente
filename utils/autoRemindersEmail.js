...
// Y luego, donde construyes el contenido del email:
...
async function sendDeliveryReminders() {
	try {
		...
		console.log(
			`FECHA: ${dayjs().tz("America/Bogota").format("DD/MM/YYYY HH:mm:ss")}`,
		);
		console.log("=================================================");
		...
		console.log(
			`Verificando entregas con fecha límite entre: ${currentDate.format("DD/MM/YYYY")} y ${dayjs(reminderDate).format("DD/MM/YYYY")}`,
		);
		...
		activePeriods.forEach((period, index) => {
			console.log(
				`  ${index + 1}. Periodo #${period.period_number} (ID: ${period.id}) - Inicio: ${dayjs(period.start_date).format("DD/MM/YYYY") || "No definido"}`,
			);
		});
		...
		console.log(
			`Total de entregas pendientes encontradas: ${pendingDeliveries.length}`,
		);

		if (pendingDeliveries.length === 0) {
			console.log(
				"No hay entregas pendientes próximas a vencer. Finalizando proceso.",
			);
			return;
		}
		...
		pendingDeliveries.forEach((delivery) => {
			const periodId =
				delivery.user_research_project.research_project.research_period_id;
		...
		for (const periodId in deliveriesByPeriod) {
			...
			periodDeliveries.forEach((delivery, index) => {
				...
				const deliveryType =
					delivery.delivery_type_id === 1 ? "AVANCE" : "ENTREGA FINAL";
				const daysLeft = dayjs(delivery.finish_date).diff(currentDate, "day");

				console.log(
					`    ${index + 1}. ${deliveryType} #${delivery.delivery_number}`,
				);
				console.log(`       Proyecto: ${project.name} (ID: ${project.id})`);
				console.log(
					`       Usuario: ${user.name} ${user.paternal_surname} ${user.maternal_surname}`,
				);
				console.log(`       Email: ${user.email || "No disponible"}`);
				console.log(
					`       Fecha límite: ${dayjs(delivery.finish_date).format("DD/MM/YYYY")} (${daysLeft} días restantes)`,
				);
			});
			...
			for (const delivery of periodDeliveries) {
				try {
					...
					if (
						!delivery.user_research_project ||
					...
					if (!user.email) {
						throw new Error(
							`El usuario ${user.name} ${user.paternal_surname} no tiene email registrado`,
						);
					}

					const deliveryType =
						delivery.delivery_type_id === 1 ? "AVANCE" : "ENTREGA FINAL";
					const daysUntilDeadline = dayjs(delivery.finish_date).diff(
						currentDate,
						"day",
					);
					...
					console.log(
						`      Para: ${user.name} ${user.paternal_surname} (${user.email})`,
					);
        ...
        `;
					...
					console.log(
						`      ✓ RECORDATORIO ENVIADO EXITOSAMENTE (MessageID: ${result.messageId})`,
					);
					periodEmailsSent++;
					totalEmailsSent++;
				} catch (emailError) {
					console.error(
						`      ✗ ERROR AL ENVIAR RECORDATORIO: ${emailError.message}`,
					);
					periodEmailsFailed++;
					totalEmailsFailed++;
				}
		...
		console.log("=================================================");
		console.log(
			`Periodos activos procesados: ${Object.keys(deliveriesByPeriod).length}`,
		);
		...
		console.log(
			`Proceso completado: ${dayjs().tz("America/Bogota").format("DD/MM/YYYY HH:mm:ss")}`,
		);
		console.log("=================================================");
	} catch (error) {
		console.error("\n=================================================");
